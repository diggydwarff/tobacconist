...
/** Loader-safe spectacles lookup. Curios remains optional. */
public final class SpectaclesEquipmentHelper {
    ...
    public static ItemStack findWorn(Player player) {
        ...
        if (head.is(ModItems.TOBACCONISTS_SPECTACLES.get())) return head;
        if (ModList.get().isLoaded("curios")) return CuriosSpectaclesHelper.findWorn(player);
