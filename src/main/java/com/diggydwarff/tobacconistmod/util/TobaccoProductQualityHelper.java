...
public final class TobaccoProductQualityHelper {
     ...
     * Recalculation uses original input metadata, making the operation idempotent if multiple
     * automation hooks observe the same craft. Dedicated Create incomplete-item production does
     * not call this helper and therefore remains full quality.
     */
    public static boolean applyGenericAutomationPenalty(ItemStack stack) {
        if (stack == null || stack.isEmpty() || !CreateCompat.loaded()) return false;

        int penalty = TobacconistConfig.getGenericAutomationQualityPenalty();
        if (penalty <= 0) return false;

        CompoundTag tag = LegacyItemTags.getTag(stack);
        if (tag == null || !tag.contains(TAG_INPUT_TOBACCO_QUALITY)) return false;
        ...
        if (stack.is(ModItems.CIGARETTE.get())) {
            penalizedScore = scoreQuality(Math.max(0, fillerQuality - penalty), cutType, ProductType.CIGARETTE);
            ...
            int wrapperQuality = tag.contains(TAG_INPUT_WRAPPER_QUALITY)
                    ...
                    : fillerQuality;
            int compositeQuality = Math.round(fillerQuality * 0.75f + wrapperQuality * 0.25f);
            penalizedScore = scoreQuality(Math.max(0, compositeQuality - penalty), cutType, ProductType.CIGAR);
        } else {
            return false;
        }
