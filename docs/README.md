# Tobacconist Gameplay Guides

These guides document the Minecraft 1.21.1 NeoForge implementation in this source tree. Values and restrictions are taken from the current Java/resource code rather than older wiki text.

- [Growing Tobacco](GROWING.md)
- [Curing Tobacco](CURING.md)
- [Fermenting and Aging](FERMENTING_AND_AGING.md)
- [Cutting and Finished Products](CUTTING_AND_PRODUCTS.md)
- [Blending, Flavoring, and Shisha](BLENDING_FLAVORING_AND_SHISHA.md)
- [Smoking, Pipes, and Hookahs](SMOKING_AND_HOOKAHS.md)
- [Storage and Packaging](STORAGE_AND_PACKAGING.md)
- [Automation and Create](AUTOMATION_AND_CREATE.md)
- [Homogenization and Quality Grading](HOMOGENIZATION_AND_GRADING.md)
- [Quality and Metadata](QUALITY_AND_METADATA.md)
- [Configuration and Debug Tools](CONFIGURATION_AND_DEBUG.md)
- [Version history](../changelog.md)

The in-game Tobacconist's Manual is the compact player-facing reference. These Markdown files include more exact thresholds for pack maintainers, server owners, testers, and contributors.
